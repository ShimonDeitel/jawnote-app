...
struct ContentView: View {
    ...
    @State private var editingEntry: JawnoteEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("Track mornings you wake with jaw soreness from grinding.")
    ...
    private func entryRow(_ entry: JawnoteEntry) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.date.formatted(date: .abbreviated, time: .shortened))
                .font(Theme.captionFont)
                .foregroundStyle(Theme.textSecondary)
            HStack {
                Text("Soreness (1-5): \(entry.value1)")
                    .font(Theme.bodyFont)
                    .foregroundStyle(Theme.textPrimary)
                Spacer()
                if !entry.value2.isEmpty {
                    Text("Stress Day (y/n): \(entry.value2)")
                        .font(Theme.captionFont)
...
struct EntryFormView: View {
    ...
    private let existing: JawnoteEntry?
    let onSave: (JawnoteEntry) -> Void

    init(entry: JawnoteEntry?, onSave: @escaping (JawnoteEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
        ...
        NavigationStack {
            Form {
                Section("Morning Log") {
                    ...
                    TextField("Soreness (1-5)", text: $value1)
                        .accessibilityIdentifier("value1Field")
                    TextField("Stress Day (y/n)", text: $value2)
